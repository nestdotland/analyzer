...
var iroh = (function () {
  ...
  pp$1.isLet = function () {
    if (
      this.type !== types.name || this.options.ecmaVersion < 6 ||
      this.value != "let"
    ) {
      return false;
    }
  ...
  pp$1.isAsyncFunction = function () {
    if (
      this.type !== types.name || this.options.ecmaVersion < 8 ||
      this.value != "async"
    ) {
      return false;
    }
  ...
  pp$1.parseClass = function (node, isStatement) {
    ...
    while (!this.eat(types.braceR)) {
      ...
      if (
        this$1.options.ecmaVersion >= 8 && !isGenerator && !method.computed &&
        method.key.type === "Identifier" && method.key.name === "async" &&
      ...
      if (!method.computed) {
        var key = method.key;
        if (
          !isGenerator && !isAsync && key.type === "Identifier" &&
          this$1.type !== types.parenL &&
    ...
    switch (expr.type) {
      case "Identifier":
        if (this.strict && this.reservedWordsStrictBind.test(expr.name)) {
          this.raiseRecoverable(
            expr.start,
            (bindingType ? "Binding " : "Assigning to ") + expr.name +
              " in strict mode",
          );
        }
        if (checkClashes) {
  ...
  pp$3.parseExprSubscripts = function (refDestructuringErrors) {
    ...
    if (
      this.checkExpressionErrors(refDestructuringErrors) || skipArrowSubscripts
    ) {
      return expr;
    }
  ...
  pp$3.parseSubscripts = function (base, startPos, startLoc, noCalls) {
    ...
    for (var computed = (void 0);;) {
      if ((computed = this$1.eat(types.bracketL)) || this$1.eat(types.dot)) {
        ...
        if (
          maybeAsyncArrow && !this$1.canInsertSemicolon() &&
  ...
  pp$3.parseParenAndDistinguishExpression = function (canBeArrow) {
    ...
    if (this.options.ecmaVersion >= 6) {
      ...
      while (this.type !== types.parenR) {
        ...
        if (
          allowTrailingComma && this$1.afterTrailingComma(types.parenR, true)
        ) {
          lastIsComma = true;
          break;
        } else if (this$1.type === types.ellipsis) {
  ...
  pp$3.parseObj = function (isPattern, refDestructuringErrors) {
    ...
    while (!this.eat(types.braceR)) {
      ...
      if (
        !isPattern && this$1.options.ecmaVersion >= 8 && !isGenerator &&
  ...
  pp$3.parseFunctionBody = function (node, isArrowFunction) {
    ...
    if (isExpression) {
      ...
      this.checkParams(
        node,
        !oldStrict && !useStrict && !isArrowFunction &&
          this.isSimpleParamList(node.params),
      );
  ...
  pp$5.canDeclareLexicalName = function (name) {
    ...
    return !has(currentScope.lexical, name) && !has(currentScope.var, name) &&
  ...
  pp$8.readToken_plus_min = function (code) { // '+-'
    ...
    if (next === code) {
      if (
        next == 45 && this.input.charCodeAt(this.pos + 2) == 62 &&
        (this.lastTokEnd === 0 ||
  ...
  pp$8.readToken_lt_gt = function (code) { // '<>'
    ...
    if (
      next == 33 && code == 60 && this.input.charCodeAt(this.pos + 2) == 45 &&
  ...
  pp$8.readRegexp = function () {
    ...
    return this.finishToken(
      types.regexp,
      { pattern: content, flags: mods, value: value },
    );
  };
    ...
    (function (global, factory) {
      ...
      var State = function () {
        function State(options) {
          ...
          this.generator = setup.generator != null
            ? setup.generator
            : baseGenerator;
          // Formating setup
      ...
      (function (exports) {
        ...
        function findNodeBefore(node, pos, test, base, state) {
          ...
          (function c(node, st, override) {
            ...
            var type = override || node.type;
            if (
              node.end <= pos && (!max || max.node.end < node.end) &&
              test(type, node)
            ) {
